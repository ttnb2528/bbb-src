...
class VideoList extends Component<VideoListProps, VideoListState> {
  ...
  setOptimalGrid() {
    ...
    if (hasPresenter) {
      ...
      const minColsForRemaining = Math.max(
        2,
        Math.ceil(Math.sqrt(remainingCams)),
      );
      ...
      optimalGrid = range(1, adjustedNumItems + 1).reduce(
        (currentGrid, col) => {
    ...
    const videoItem = (
      <VideoListItemContainer
        ...
        onVirtualBgDrop={(type, name, data) => {
          return isStream
            ? onVirtualBgDrop(item.stream, type, name, data)
