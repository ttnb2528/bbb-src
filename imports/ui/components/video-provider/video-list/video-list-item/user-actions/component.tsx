...
const UserActions: React.FC<UserActionProps> = (props) => {
  ...
  const getAvailableActions = () => {
    ...
    const isCameraDisabled =
      Array.isArray(disabledCams) && disabledCams?.includes(cameraId);
    const enableSelfCamIntlKey = !isCameraDisabled ? "disable" : "enable";
    const ALLOW_FULLSCREEN = !isIphone
    ...
    const toggleDisableCam = () => {
      if (!isCameraDisabled) {
        ...
        notify(
          intl.formatMessage(intlMessages.disableWarning),
          "info",
          "warning",
        );
      } else {
        Session.setItem(
          "disabledCams",
          disabledCams.filter((cId: string) => cId !== cameraId),
        );
      }
    };
    ...
    if (userId === Auth.userID && isStream && !isSelfViewDisabled) {
      menuItems.push({
        ...
        description: intl.formatMessage(
          intlMessages[`${enableSelfCamIntlKey}Label`],
        ),
    ...
    if (isStream) {
      menuItems.push({
        ...
        description: intl.formatMessage(
          intlMessages[`${isMirroredIntlKey}Desc`],
        ),
    ...
    userCameraDropdownItems
      .filter(
        (pluginItem) =>
          pluginItem.displayFunction?.({ userId, streamId: cameraId }) ?? true,
      )
      .forEach((pluginItem) => {
