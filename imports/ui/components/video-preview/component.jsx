...
const intlMessages = defineMessages({
  ...
  TypeError: {
    ...
    description:
      "List of constraints specified is empty, or has all constraints set to false",
  },
  NotFoundError: {
  ...
  NotReadableError: {
    ...
    description:
      "error message When the webcam is being used by other software",
  },
  TimeoutError: {
  ...
  genericError: {
    ...
    description:
      "error message for when the webcam sharing fails with unknown error",
  },
  camCapReached: {
  ...
  cameraAsContentSettingsTitle: {
    ...
    description:
      "Title for the video preview modal when sharing camera as content",
  },
  sliderDesc: {
...
class VideoPreview extends Component {
  ...
  constructor(props) {
    ...
    this.handleBrightnessAreaChange =
      this.handleBrightnessAreaChange.bind(this);
  ...
  set currentVideoStream(bbbVideoStream) {
    ...
    if (this._currentVideoStream) {
      this._currentVideoStream.removeListener(
        "inactive",
        this.handleLocalStreamInactive,
      );
    }
  ...
  componentDidMount() {
    const { webcamDeviceId, forceOpen } = this.props;
    ...
    const populatePreview = ({
      ...
      const initialDeviceId =
        processedCamerasList[0]?.deviceId || webcamDeviceId;

      this.getInitialCameraStream(initialDeviceId).then(async () => {
        ...
        if (!areLabelled || !areIdentified) {
          ...
          try {
            const { devices: newDevices, digestedWebcams: newDigestedWebcams } =
              await PreviewService.doEnumerateDevices({
                priorityDeviceId: webcamDeviceId,
              });
    ...
    if (deviceInfo.hasMediaDevices) {
      const SKIP_INITIAL_ENUM =
        window.meetingClientSettings.public.media.skipInitialCamEnumeration;
  ...
  async startCameraBrightness(initialState = DEFAULT_BRIGHTNESS_STATE) {
    ...
    const CAMERA_BRIGHTNESS_AVAILABLE =
      ENABLE_CAMERA_BRIGHTNESS && isVirtualBackgroundSupported();
    ...
    if (CAMERA_BRIGHTNESS_AVAILABLE && this.currentVideoStream) {
      ...
      const applyStreamBrightnessState = () => {
        ...
        this.currentVideoStream.toggleCameraBrightnessArea(
          initialState.wholeImageBrightness,
        );
      };
  ...
  async handleStartSharing() {
    ...
    const { webcamDeviceId, selectedProfile, brightness } = this.state;
  ...
  handleVirtualBgError(error, type, name) {
    ...
    notify(
      intl.formatMessage(intlMessages.virtualBgGenericError),
      "error",
      "video",
    );
  }
  ...
  applyStoredVirtualBg(deviceId = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (virtualBackground) {
        ...
        const { filename, data, type, uniqueId } =
          this.context.backgrounds.webcamBackgroundURL;
        const customParams = {
          file: data,
          uniqueId,
        };
        ...
        this.handleVirtualBgSelected(type, filename, customParams).then(
          resolve,
          handleFailure,
        );
      } else {
  ...
  async applyStoredBrightness(deviceId = null) {
    ...
    if (
      cameraBrightness &&
      !isEqual(cameraBrightness, DEFAULT_BRIGHTNESS_STATE)
    ) {
  ...
  skipVideoPreview() {
    ...
    return this.getInitialCameraStream(webcamDeviceId)
      .then(() => {
  ...
  renderDeviceSelectors() {
    const { intl, sharedDevices, cameraAsContent } = this.props;

    const { webcamDeviceId, availableWebcams, selectedProfile } = this.state;
  ...
  renderQualitySelector() {
    ...
    const { selectedProfile, availableWebcams, webcamDeviceId } = this.state;
    ...
    return (
      <>
        ...
        {PREVIEW_CAMERA_PROFILES.length > 0 ? (
          ...
          <span>{intl.formatMessage(intlMessages.profileNotFoundLabel)}</span>
        )}
      </>
    );
  }
  ...
  renderBrightnessInput() {
    const { cameraAsContent, cameraAsContentDeviceId } = this.props;
    const { webcamDeviceId } = this.state;
    ...
    const { intl } = this.props;
    const { brightness, wholeImageBrightness, isStartSharingDisabled } =
      this.state;
    const shared = this.isAlreadyShared(webcamDeviceId);
        ...
        <div style={{ display: "flex", marginTop: ".5rem" }}>
          <Checkbox
            ...
            ariaLabel={intl.formatMessage(
              intlMessages.wholeImageBrightnessLabel,
            )}
  ...
  renderVirtualBgSelector() {
    ...
    const { showThumbnails: SHOW_THUMBNAILS = true } =
  ...
  renderTabsContent(tabNumber) {
    const { cameraAsContent, isVirtualBackgroundsEnabled } = this.props;

    const shouldShowVirtualBackgrounds =
    ...
    return (
      <Styled.ContentCol>
        ...
        {tabNumber === 1 && shouldShowVirtualBackgrounds && (
          <Styled.BgnCol>{this.renderVirtualBgSelector()}</Styled.BgnCol>
        )}
      </Styled.ContentCol>
    );
  }
  ...
  renderContent(selectedTab) {
    ...
    const { viewState, deviceError, previewError } = this.state;
            ...
            <Styled.VideoCol>
              {previewError ? (
                ...
                <Styled.VideoPreview
                  ...
                  id="preview"
                  data-test={
                    VideoService.mirrorOwnWebcam()
                      ? "mirroredVideoPreview"
                      : "videoPreview"
                  }
                  ref={(ref) => {
                    this.video = ref;
  ...
  renderModalContent(selectedTab) {
    const { intl, hasVideoStream, forceOpen, camCapReached, closeModal } =
      this.props;
    ...
    const shouldDisableButtons =
      this.shouldSkipVideoPreview() || !!(deviceError || previewError);

    const shared = this.isAlreadyShared(webcamDeviceId);

    const showStopAllButton =
              ...
              <div style={{ display: "flex" }}>
                <Styled.CancelButton
                  ...
                  label={intl.formatMessage(
                    shared ? intlMessages.closeLabel : intlMessages.cancelLabel,
                  )}
                  onClick={closeModal}
                />
                {(!shared || selectedTab === 0) && (
                  <Styled.SharingButton
                    data-test="startSharingWebcam"
                    color={shared ? "danger" : "primary"}
                    label={intl.formatMessage(
                      shared
                        ? intlMessages.stopSharingLabel
                        : intlMessages.startSharingLabel,
                    )}
                    onClick={
                      shared ? this.handleStopSharing : this.handleStartSharing
                    }
                    disabled={
                      isStartSharingDisabled ||
  ...
  render() {
    ...
    const isBlurred =
      Session.getItem("videoPreviewFirstOpen") &&
      getFromUserSettings(
        "bbb_auto_share_webcam",
        window.meetingClientSettings.public.kurento.autoShareWebcam,
      );
    ...
    const { deviceError, previewError } = this.state;
    ...
    const shouldShowVirtualBackgroundsTab =
      isVirtualBackgroundsEnabled &&
      !cameraAsContent &&
                  ...
                  <>
                    ...
                    <Styled.WebcamTabSelector selectedClassName="is-selected">
                      <Styled.IconSvg
                        ...
                        darkThemeState={darkThemeState}
                      />
                      <span id="backgrounds-title">
                        {intl.formatMessage(
                          intlMessages.webcamVirtualBackgroundTitle,
                        )}
                      </span>
                    </Styled.WebcamTabSelector>
                  </>
