...
const AppContainer = (props) => {
  ...
  const { partialUtterances, minUtteranceLength } = useSettings(
  ...
  const { hideNotificationToasts } = layoutSelectInput(
    (i) => i.notificationsBar,
  );
  ...
  const isSharedNotesPinned =
    isSharedNotesPinnedFromGraphql && presentation.isOpen;
  ...
  const [setPresentationFitToWidth] = useMutation(
  ...
  return currentUser?.userId ? (
    <App
      {...{
        ...
        metadata: currentMeeting?.metadata ?? {},
      }}
      {...props}
    />
  ) : null;
