...
import {
  ActionsBarItemType,
  ActionsBarPosition,
} from "bigbluebutton-html-plugin-sdk/dist/cjs/extensible-areas/actions-bar-item/enums";
...
class ActionsBar extends PureComponent {
  constructor(props) {
    ...
    this.renderPluginsActionBarItems =
      this.renderPluginsActionBarItems.bind(this);
    this.setModalIsOpen = this.setModalIsOpen.bind(this);
    this.handleTogglePrivateChat = this.handleTogglePrivateChat.bind(this);
    this.handleExternalOpenPrivateChat =
      this.handleExternalOpenPrivateChat.bind(this);
    this.handleCloseNotificationPanel =
  ...
  componentDidMount() {
    ...
    window.addEventListener(
      "clickMinimizedChatIcon",
      this.handleClickMinimizedChatIcon,
    );
  ...
  handleExternalOpenPrivateChat = (e) => {
    ...
    if (chatId) {
      ...
      this.setState((prevState) => {
        ...
        const minimizedChatIds = allChatIds.filter(
          (id) => prevState.openPrivateChats[id]?.isMinimized,
        );
    ...
    if (userId) {
      window.dispatchEvent(
        new CustomEvent("findChatIdFromUserId", {
          detail: { userId },
        }),
      );
    } else {
  ...
  handleExpandPrivateChat = (chatId, iconPosition = null) => {
    ...
    this.setState((prevState) => {
      ...
      const currentIconPosition =
        iconPosition || chatState.dockPosition || chatState.position;
      ...
      expandPosition.left = Math.max(
        16,
        Math.min(expandPosition.left, window.innerWidth - modalWidth - 16),
      );
      expandPosition.top = Math.max(
        16,
        Math.min(expandPosition.top, window.innerHeight - modalHeight - 16),
      );
    ...
    this.setState((prevState) => {
      ...
      const newChats = {
        ...
        [chatId]: {
          ...
          dockPosition: chatState.isMinimized
            ? position
            : chatState.dockPosition,
  ...
  handleToggleChat = () => {
    ...
    if (
      sidebarContent.isOpen &&
      sidebarContent.sidebarContentPanel === PANELS.CHAT
    ) {
      layoutContextDispatch({
        type: ACTIONS.SET_SIDEBAR_CONTENT_IS_OPEN,
        value: false,
          ...
          .map((plugin) => {
            ...
            switch (plugin.type) {
              case ActionsBarItemType.BUTTON:
                ...
                if (
                  plugin?.icon &&
                  typeof plugin.icon === "object" &&
                  "iconName" in plugin.icon
                ) {
                  buttonProps.icon = plugin.icon.iconName;
                } else if (
                  plugin?.icon &&
                  typeof plugin.icon === "object" &&
                  "svgContent" in plugin.icon
                ) {
                  buttonProps.customIcon = <i>{plugin.icon.svgContent}</i>;
                }
                actionBarItemToReturn = <Button {...buttonProps} />;
                break;
      ...
      shouldRenderActionBar && (
        ...
        >
          <h2 className="sr-only">
            {intl.formatMessage(intlMessages.actionsBarLabel)}
          </h2>
          <Styled.ActionsBar
