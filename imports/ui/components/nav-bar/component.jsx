...
const intlMessages = defineMessages({
  ...
  sessionControlLabel: {
    ...
    description:
      "label for screen reader to jump to leave button and options menu",
  },
  speakersListLabel: {
...
const propTypes = {
  ...
  pluginNavBarItems: PropTypes.arrayOf(
    PropTypes.shape({
      id: PropTypes.string,
    }),
  ).isRequired,
                ...
                returnComponent = (
                  <>
                    ...
                    <Styled.PluginSeparatorWrapper
                      key={`${pluginItem.id}-${pluginItem.type}-separator`}
                    >
                      |
                    </Styled.PluginSeparatorWrapper>
                  </>
...
class NavBar extends Component {
  constructor(props) {
    ...
    const ShownId = getStorageSingletonInstance().getItem(
      "alreadyShowSessionDetailsOnJoin",
    );

    this.state = {
      isModalOpen:
        props.showSessionDetailsOnJoin && !(ShownId === props.meetingId),
    };
  }
  ...
  setModalIsOpen(isOpen) {
    if (!isOpen) {
      ...
      getStorageSingletonInstance().setItem(
        "alreadyShowSessionDetailsOnJoin",
        meetingId,
      );
    }
  ...
  splitPluginItems() {
    ...
    return pluginNavBarItems.reduce(
      (result, item) => {
  ...
  render() {
    ...
    const { leftPluginItems, centerPluginItems, rightPluginItems } =
      this.splitPluginItems();

    const Settings = getSettingsSingletonInstance();
    const { selectedLayout } = Settings.application;
    const shouldShowNavBarToggleButton =
    ...
    return (
      shouldShowNavbar && (
        ...
        >
          {!hideTopRow && (
            <Styled.Top>
              ...
              <Styled.Center>
                ...
                >
                  <Tooltip
                    title={intl.formatMessage(intlMessages.openDetailsTooltip)}
                  >
                    <span>
                      {presentationTitle}
              ...
              <Styled.Right>
                <h2 className="sr-only">
                  {intl.formatMessage(intlMessages.sessionControlLabel)}
                </h2>
                {renderPluginItems(rightPluginItems)}
                {/* ĐÃ ẨN: Nút ! (Connection Status) dư thừa, vì đã gộp tính năng Info vào click tên Room
          ...
          <Styled.Bottom>
            <h2 className="sr-only">
              {intl.formatMessage(intlMessages.speakersListLabel)}
            </h2>
            {enableTalkingIndicator ? (
