...
interface ChatMessageProps {
  ...
  setRenderedChatMessages: React.Dispatch<
    React.SetStateAction<MessageDetails[]>
  >;
...
const ChatMessage = React.forwardRef<ChatMessageRef, ChatMessageProps>(
  (
    ...
    const [isToolbarReactionPopoverOpen, setIsToolbarReactionPopoverOpen] =
    ...
    const disablePublicChat =
      meetingDisablePublicChat || currentUserDisablePublicChat;
    ...
    const startScrollAnimation = (timestamp: number) => {
      ...
      animationInitialScrollPosition.current =
        scrollRef.current?.scrollTop || 0;
      animationScrollPositionDiff.current =
        (scrollRef.current?.scrollTop || 0) -
        ((containerRef.current?.offsetTop || 0) -
          (scrollRef.current?.offsetHeight || 0) / 2);
    ...
    const startBackgroundAnimation = (timestamp: number) => {
      animationInitialTimestamp.current = timestamp;
      animationInitialBgColor.current =
    ...
    const animateScrollPosition = (timestamp: number) => {
      const value =
        (timestamp - animationInitialTimestamp.current) /
    ...
    const animateBackgroundColor = (timestamp: number) => {
      ...
      const value =
        (timestamp - animationInitialTimestamp.current) / ANIMATION_DURATION;
      if (value < 1) {
        messageContentRef.current.style.backgroundColor = `rgb(${colorBlueLighterChannel} / ${1 - value})`;
        requestAnimationFrame(animateBackgroundColor);
      } else {
        messageContentRef.current.style.backgroundColor =
          animationInitialBgColor.current;
      }
    };
    ...
    useEffect(() => {
      setRenderedChatMessages((messages) => {
        if (
          ...
          !messages.some((m) => m.messageId === message.messageId)
        ) {
          messages.push({
            messageId: message.messageId,
    ...
    useEffect(() => {
      ...
      // Đơn giản: chỉ mark as seen khi message vào viewport
      // Logic check panel open được xử lý ở parent component
      if (message && scrollRef.current && messageRef.current) {
        if (isInViewport(messageRef.current)) {
          markMessageAsSeen(message);
        } else {
          // Nếu message chưa trong viewport, add scroll listener để check sau
    ...
    const pluginMessageNotCustom =
      previousMessage?.messageType !== ChatMessageType.PLUGIN ||
      !JSON.parse(previousMessage?.messageMetadata).custom;
    let sameSender =
      (previousMessage?.user?.userId || lastSenderPreviousPage) ===
        message?.user?.userId && pluginMessageNotCustom;
    const isSystemSender = SYSTEM_SENDERS.has(
      message.messageType as ChatMessageType,
    );
    ...
    const messageContent: {
      ...
      switch (message.messageType) {
        case ChatMessageType.POLL: {
          const pollData = JSON.parse(message.messageMetadata) as {
            quiz: boolean;
          };
          return {
            name: pollData.quiz
              ? intl.formatMessage(intlMessages.quizResult)
              : intl.formatMessage(intlMessages.pollResult),
            color: "#6366F1", // Indigo
            isModerator: true,
            component: <ChatPollContent metadata={message.messageMetadata} />,
        ...
        case ChatMessageType.BREAKOUT_ROOM:
          return {
            ...
            isModerator: true,
            isSystemSender: true,
            component: <ChatMessageTextContent text={message.message} />,
            showAvatar: true,
            showHeading: true,
            showToolbar: true,
          };
        ...
        case ChatMessageType.PLUGIN: {
          return {
            ...
            showHeading: true,
            showToolbar: true,
            component: currentPluginMessageMetadata.custom ? null : (
              <ChatMessageTextContent text={message.message} />
            ),
          };
        }
    ...
    sameSender =
      message.messageType === ChatMessageType.BREAKOUT_ROOM
        ? false
        : (previousMessage?.user?.userId || lastSenderPreviousPage) ===
            message?.user?.userId && pluginMessageNotCustom;

    // Chỉ hiển thị avatar và header khi không cùng sender với tin nhắn trước (giống Zalo/Facebook Messenger)
    // Trong modal mode, vẫn hiển thị header để phân biệt ai gửi, nhưng avatar chỉ hiển thị một lần
    const shouldRenderAvatar =
      messageContent.showAvatar && !sameSender && !isCustomPluginMessage;

    const shouldRenderHeader =
      messageContent.showHeading &&
      ...
      >
        ...
        {!deleteTime && (
          <MessageItemWrapper>
            ...
            {messageReadFeedbackEnabled && (
              <MessageReadConfirmation message={message} />
            )}
          </MessageItemWrapper>
        )}
        {sameSender && (
        ...
        {deleteTime && (
          <DeleteMessage>
            {intl.formatMessage(intlMessages.deleteMessage, {
              userName: message.deletedBy?.name,
            })}
          </DeleteMessage>
        )}
      </ChatContent>
    ...
    const reactionsPopover = (
      <Popover
        ...
        anchorOrigin={{
          vertical: "top",
          horizontal: isRTL || isMobile || mode === "modal" ? "left" : "right",
        }}
        transformOrigin={{
          vertical: "top",
          horizontal: isRTL || mode === "modal" ? "right" : "left",
        }}
      >
        <EmojiPickerWrapper>
          <EmojiPicker
    ...
    const focusable =
      ...
      (!messageContent.isSystemSender ||
        message.messageType === ChatMessageType.POLL);
        ...
        {isTryingToDelete && (
          <ConfirmationModal
            ...
            description={intl.formatMessage(
              intlMessages.confirmationDescription,
            )}
