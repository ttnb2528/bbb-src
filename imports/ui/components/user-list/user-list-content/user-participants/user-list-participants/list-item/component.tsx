...
const renderUserListItemIconsFromPlugin = (
  ...
  userItemsFromPlugin
    .filter((item) => item.type === UserListItemAdditionalInformationType.ICON)
...
const UserListItem: React.FC<UserListItemProps> = ({
  ...
  if (pluginsExtensibleAreasAggregatedState.userListItemAdditionalInformation) {
    userItemsFromPlugin =
      pluginsExtensibleAreasAggregatedState.userListItemAdditionalInformation.filter(
        (item) => {
          const userListItem =
            item as PluginSdk.UserListItemAdditionalInformationInterface;
  ...
  const isConnectedFromAudioManager = useReactiveVar(
    AudioManager._isConnected.value,
  ) as boolean;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const isDeafenedFromAudioManager = useReactiveVar(
    AudioManager._isDeafened.value,
  ) as boolean;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const isListenOnlyFromAudioManager = useReactiveVar(
    AudioManager._isListenOnly.value,
  ) as boolean;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const inputDeviceIdFromAudioManager = useReactiveVar(
    AudioManager._inputDeviceId.value,
  ) as string | null;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const isMutedFromAudioManager = useReactiveVar(
  ...
  const listenOnlyFromState = isCurrentUser
    ? isListenOnlyFromAudioManager ||
      inputDeviceIdFromAudioManager === "listen-only"
    : user.voice?.listenOnly ||
      user.voice?.listenOnlyInputDevice ||
      user?.listenOnly;
  ...
  if (
    (user.locked || user.userLockSettings?.disablePublicChat) &&
      ...
      <span key={uniqueId("breakout-")}>
        ...
        {user.lastBreakoutRoom?.shortName
          ? intl.formatMessage(messages.breakoutRoom, {
              roomNumber: user.lastBreakoutRoom?.sequence,
            })
          : user.lastBreakoutRoom?.shortName}
      </span>,
    );
  ...
  userItemsFromPlugin
    .filter((item) => item.type === UserListItemAdditionalInformationType.LABEL)
    .forEach((item) => {
      const itemToRender = item as PluginSdk.UserListItemLabel;
      subs.push(
        <span key={itemToRender.id}>
          {itemToRender.icon && (
            <Styled.UserAdditionalInformationIcon
  ...
  const userAvatarFiltered =
    user?.raiseHand === true ||
    user.away === true ||
    (user.reactionEmoji && user.reactionEmoji !== "none")
      ? ""
      : user.avatar;
  ...
  const avatarContent =
    user.lastBreakoutRoom?.currentlyInRoom && userAvatarFiltered.length === 0
  ...
  return (
    <Styled.UserItemContents
      ...
      tabIndex={-1}
      data-test={
        user.userId === Auth.userID ? "userListItemCurrent" : "userListItem"
      }
      role="listitem"
    >
      <Styled.Avatar
