...
import GET_TIMER, {
  GetTimerResponse,
  TimerData,
} from "../../../core/graphql/queries/timer";
...
const TimerPanel: React.FC<TimerPanelProps> = ({
  ...
  const setSeconds = useCallback((seconds: number, time: number) => {
    if (!Number.isNaN(seconds) && seconds >= 0 && seconds <= 59) {
      ...
      const currentSeconds = Math.floor(
        (time - mHours - mMinutes) / MILLI_IN_SECOND,
      );
              ...
              {TRACKS.map((track) => (
                <Styled.TimerTrackItem key={track}>
                  <label htmlFor={track}>
                    ...
                    />
                    {intl.formatMessage(
                      intlMessages[track as keyof typeof intlMessages],
                    )}
                  </label>
  ...
  return (
    ...
    >
      ...
      />
      <Styled.TimerContent>
        <Styled.TimerCurrent aria-hidden data-test="timerCurrent">
...
const TimerPanelContaier: React.FC = () => {
  ...
  const timeDifferenceMs: number =
    adjustedCurrent.getTime() - startedAtDate.getTime();

  const timePassed = timer.stopwatch
    ? Math.floor((timer.running ? timeDifferenceMs : 0) + timer.accumulated)
    : Math.floor(
        timer.time -
          (timer.accumulated + (timer.running ? timeDifferenceMs : 0)),
      );
