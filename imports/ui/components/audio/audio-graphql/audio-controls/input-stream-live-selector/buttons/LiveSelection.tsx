...
import {
  ...
  MenuOptionItemType,
} from "/imports/ui/components/common/menu/menuTypes";
...
export const LiveSelection: React.FC<LiveSelectionProps> = ({
  ...
  console.log(
    "[LiveSelection] Render - inputDeviceId:",
    inputDeviceId,
    "listenOnly (prop):",
    listenOnly,
    "shouldTreatAsMicrophone:",
    inputDeviceId !== "listen-only" &&
      (!listenOnly || supportsTransparentListenOnly),
  );
  ...
  let audioSettingsDropdownItems =
    [] as PluginSdk.AudioSettingsDropdownInterface[];
  ...
  const renderDeviceList = useCallback(
    (
      ...
      if (listLength > 0) {
        deviceList = list.map((device, index) => {
          ...
          return {
            ...
            label: truncateDeviceName(
              device.label || getFallbackLabel(device, index + 1),
            ),
            customStyles: isCurrentDevice
              ? Styled.SelectedLabel
              : Styled.DeviceLabel,
            iconStyles: isCurrentDevice ? Styled.SelectedLabelIcon : null,
            iconRight: isCurrentDevice ? "check" : null,
            onClick: () =>
              onDeviceListClick(device.deviceId, deviceKind, callback),
          } as MenuOptionItemType;
        });
      } else if (
        deviceKind === AUDIO_OUTPUT &&
        !SET_SINK_ID_SUPPORTED &&
        listLength === 0
      ) {
      ...
      if (deviceKind === AUDIO_INPUT && supportsTransparentListenOnly) {
        ...
        const listenOnly =
          deviceKind === AUDIO_INPUT && currentDeviceId === "listen-only";
  ...
  const onDeviceListClick = useCallback(
    (deviceId: string, deviceKind: string, callback: Function) => {
      if (!deviceId) {
  ...
  audioSettingsDropdownItems.forEach(
    (audioSettingsDropdownItem: PluginSdk.AudioSettingsDropdownInterface) => {
      switch (audioSettingsDropdownItem.type) {
        case AudioSettingsDropdownItemType.OPTION: {
          const audioSettingsDropdownOption =
            audioSettingsDropdownItem as PluginSdk.AudioSettingsDropdownOption;
        ...
        case AudioSettingsDropdownItemType.SEPARATOR: {
          const audioSettingsDropdownSeparator =
            audioSettingsDropdownItem as PluginSdk.AudioSettingsDropdownOption;
          dropdownListComplete.push({
            isSeparator: true,
