...
const InputStreamLiveSelector: React.FC<InputStreamLiveSelectorProps> = ({
  ...
  const { enableDynamicAudioDeviceSelection } =
  ...
  useEffect(() => {
    ...
    if (
      inputDeviceId === "listen-only" &&
      isConnected &&
      !isMutedFromAudioManager
    ) {
      // eslint-disable-next-line no-console
      console.log(
        "[InputStreamLiveSelectorContainer] Auto-muting for listen-only mode",
      );
...
const InputStreamLiveSelectorContainer: React.FC<
  InputStreamLiveSelectorContainerProps
> = ({ openAudioSettings }) => {
  // eslint-disable-next-line no-console
  ...
  const talking = Boolean(
    currentUser?.userId && talkingUsers[currentUser.userId],
  );
  ...
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const isMutedVar = AudioManager._isMuted?.value;
  const isMutedFromAudioManager = isMutedVar
    ? (useReactiveVar(isMutedVar) as boolean)
    : null;
  // Get GraphQL muted state for comparison
  const mutedFromGraphQL = Boolean(
  ...
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const isConnecting = useReactiveVar(
    AudioManager._isConnecting.value,
  ) as boolean;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const isHangingUp = useReactiveVar(
    AudioManager._isHangingUp.value,
  ) as boolean;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  ...
  const [inputDeviceId, setInputDeviceId] = React.useState(() => {
    ...
    // eslint-disable-next-line no-console
    console.log(
      "[InputStreamLiveSelectorContainer] Initial state:",
      initialValue,
    );
    return initialValue;
  });
  ...
  React.useEffect(() => {
    // eslint-disable-next-line no-console
    console.log(
      "[InputStreamLiveSelectorContainer] useEffect mounted, setting up polling",
    );
    ...
    const checkAndUpdate = () => {
      const currentValue = (AudioManager._inputDeviceId?.value() ||
        "") as string;
      if (currentValue !== previousValue) {
        // eslint-disable-next-line no-console
        console.log(
          "[InputStreamLiveSelectorContainer] Reactive var changed, updating state:",
          {
    ...
    return () => {
      // eslint-disable-next-line no-console
      console.log(
        "[InputStreamLiveSelectorContainer] useEffect cleanup, clearing interval",
      );
  ...
  const reactiveVarValue = (useReactiveVar(AudioManager._inputDeviceId.value) ||
    "") as string;
  ...
  React.useEffect(() => {
    if (reactiveVarValue !== inputDeviceId) {
      // eslint-disable-next-line no-console
      console.log(
        "[InputStreamLiveSelectorContainer] ReactiveVar value differs from state, syncing:",
        {
  ...
  // eslint-disable-next-line no-console
  console.log(
    "[InputStreamLiveSelectorContainer] Render - inputDeviceId:",
    inputDeviceId,
    "reactiveVarValue:",
    reactiveVarValue,
    "listenOnly (GraphQL):",
    currentUser?.voice?.listenOnly ?? false,
    "timestamp:",
    new Date().toISOString(),
  );
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const outputDeviceId = useReactiveVar(
    AudioManager._outputDeviceId.value,
  ) as string;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const inputStream = useReactiveVar(AudioManager._inputStream) as string;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  const permissionStatus = useReactiveVar(
    AudioManager._permissionStatus.value,
  ) as string;
  // @ts-ignore - temporary while hybrid (meteor+GraphQl)
  ...
  React.useEffect(() => {
    if (
      isMutedVar &&
      isMutedFromAudioManager !== null &&
      isMutedFromAudioManager !== mutedFromGraphQL
    ) {
      ...
      try {
        const Session =
          require("/imports/ui/services/storage/in-memory").default;
      ...
      if (
        ...
        !mutedFromGraphQL &&
        isConnected &&
        timeSinceLastToggle > TOGGLE_SYNC_DELAY &&
        !wasAutoJoin
      ) {
    ...
    <InputStreamLiveSelector
      ...
      isAudioLocked={
        (!currentUser?.isModerator &&
          currentUser?.locked &&
          currentMeeting?.lockSettings?.disableMic) ??
        false
      }
