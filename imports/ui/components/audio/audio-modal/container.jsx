...
const AudioModalContainer = (props) => {
  ...
  const listenOnlyMode =
    forceListenOnly ||
    (getFromUserSettings("bbb_listen_only_mode", APP_CONFIG.listenOnlyMode) &&
      !usingLiveKit);
  const skipCheck = getFromUserSettings(
    "bbb_skip_check_audio",
    APP_CONFIG.skipCheck,
  );
  const skipCheckOnJoin = getFromUserSettings(
  ...
  const skipEchoTestIfPreviousDevice =
    getFromUserSettings(
  ...
  const showPermissionsOvelay = useReactiveVar(
    AudioManager._isWaitingPermissions.value,
  );
  ...
  const effectiveInputDeviceId =
    !isUsingAudio && reactiveInputDeviceId === "listen-only"
      ? getStoredAudioInputDeviceId() || ""
      : reactiveInputDeviceId;
  const devicesAlreadyConfigured =
    skipEchoTestIfPreviousDevice && !!getStoredAudioInputDeviceId();
  const joinFullAudioImmediately =
    !isListenOnlyInputDevice &&
    (skipCheck ||
    ...
    const handleJoinError = (error, listenOnly) => {
      if (
        ...
        (error.name === "NotAllowedError" ||
          error.errCode === AudioError.MIC_ERROR.NO_PERMISSION)
      ) {
