...
import { GeoShapeGeoStyle } from "@bigbluebutton/editor";
...
const Whiteboard = React.memo((props) => {
  ...
  const customUiOverrides = React.useMemo(
    () => ({
      ...
      toolbar: (_editor, toolbarItems, { tools }) => {
        ...
        if (bbbPresenterTools.length >= 1 && isPresenter) {
          return toolbarItems.filter((item) =>
            bbbPresenterTools.includes(item.id),
          );
        }
        ...
        if (bbbMultiUserTools.length >= 1 && !isModerator) {
          return toolbarItems.filter((item) =>
            bbbMultiUserTools.includes(item.id),
          );
        }
        // full toolbar
        return toolbarItems;
      ...
      translations: ["en", intl.locale, intl.locale.split("-")[0]].reduce(
        (acc, locale) => {
          acc[locale] = {
  ...
  React.useEffect(() => {
    ...
    const toolbarSavedState = SessionStorage.getItem(
      "whiteboardToolbarSavedState",
    );
    ...
    if (!hasWBAccess && !isPresenter) {
      ...
      if (toolbarSavedState) {
        const { selectedTool: savedSelectedTool } = toolbarSavedState;
  ...
  const handleKeybindZoom = useCallback(
    (operation) => {
      if (
        !tlEditorRef.current ||
      ...
      if (operation === "zoomIn") {
        currentZoomLevel = Math.min(
          currentZoomLevel + ZOOM_IN_FACTOR,
          MAX_ZOOM_FACTOR,
        );
      } else {
        currentZoomLevel = Math.max(
          currentZoomLevel - ZOOM_OUT_FACTOR,
          MIN_ZOOM_FACTOR,
        );
      }
  ...
  const handleKeyDown = useCallback(
    (event) => {
      ...
      const simpleKeyMap = {
        ...
        4: () => {
          tlEditorRef.current?.setStyleForNextShapes(
            GeoShapeGeoStyle,
            "rectangle",
          );
        ...
        "[": () => {
          tlEditorRef.current?.sendBackward(
            tlEditorRef.current?.getSelectedShapes(),
          );
        },
        "]": () => {
          tlEditorRef.current?.bringForward(
            tlEditorRef.current?.getSelectedShapes(),
          );
        },
      ...
      if (event.shiftKey && !event.ctrlKey && !event.metaKey && !event.altKey) {
        const shiftKeyMap = {
          ...
          h: () => {
            tlEditorRef.current?.flipShapes(
              tlEditorRef.current?.getSelectedShapes(),
              "horizontal",
            );
          },
          ...
          "}": () => {
            tlEditorRef.current?.bringToFront(
              tlEditorRef.current?.getSelectedShapes(),
            );
          },
          "{": () => {
            tlEditorRef.current?.sendToBack(
              tlEditorRef.current?.getSelectedShapes(),
            );
          },
          "!": () => {
          ...
          "@": () => {
            const selectionBounds =
              tlEditorRef.current?.getSelectionPageBounds();
            ...
            const adjustedBaseZoomToFitIn =
              Math.max(baseZoomToFitIn, initialZoomRef.current) /
              initialZoomRef.current;
      ...
      if (event.ctrlKey || event.metaKey) {
        ...
        if (key === "l" && event.shiftKey) {
          event.preventDefault();
          event.stopPropagation();
          tlEditorRef.current?.toggleLock(
            tlEditorRef.current?.getSelectedShapes(),
          );
          return;
        }
  ...
  const coreCameraLogic = ({ baseZoom, xOffset, yOffset, description }) => {
    ...
    if (Number.isNaN(formattedPageId)) {
      throw new Error(
        `Invalid formattedPageId ${description}: ${formattedPageId}`,
      );
    }
  ...
  const calculateZoomWithGapValue = (
    ...
    const presentationWidthLocal = Math.max(
      presentationAreaWidth - widthAdjustment,
      0,
    );
    const calcedZoom = fitToWidth
    ...
    try {
      ...
      if (
        ...
        if (isPresenterRef.current) {
          ...
          if (widthGap > 0) {
            const zoomWithGap = calculateZoomWithGapValue(
              scaledWidth,
              scaledHeight,
              widthGap,
            );
  ...
  const handleTldrawMount = (editor) => {
    if (typeof editor.history.setMaxStackSize === "function") {
      editor.history.setMaxStackSize(
        window.meetingClientSettings.public.whiteboard.maxHistoryStackSize,
      );
    } else {
      logger.warn(
        { logCode: "SetMaxStackSize" },
        "Failed to set max history stack size - feature not available",
      );
    }
    ...
    editor.store.listen(
      (entry) => {
        ...
        Object.values(updated).forEach(([, record]) => {
          ...
          const createdBy =
            formattedLookup[record?.id]?.meta?.createdBy || currentUser?.userId;
          const updatedRecord = {
            ...record,
            meta: {
              ...record.meta,
          ...
          const diff = getDifferences(
            prevShapesRef.current[record?.id],
            updatedRecord,
          );
          if (diff) {
            diff.id = record.id;
        ...
        if (cameras) {
          ...
          if (
            ...
            if (
              ...
              (!hasZoomSyncedRef.current ||
                (hasZoomSyncedRef.current && panned)) &&
    ...
    if (editor && curPageIdRef.current) {
      ...
      if (!isPresenterRef.current && !hasWBAccessRef.current) {
        ...
        if (isPresenterRef.current) {
          const initialPresenterTool = presenterTools.includes(
            initialSelectedTool,
          )
            ? initialSelectedTool
            : "noop";
          ...
          const initialTool = multiUserTools.includes(initialSelectedTool)
            ? initialSelectedTool
            : "noop";
  ...
  const syncCameraWithPresentationArea = () => {
    ...
    if (isPresenter) {
      ...
      if (widthGap > 0) {
        const gapZoom =
          calculateZoomWithGapValue(scaledWidth, scaledHeight, widthGap) ||
          HUNDRED_PERCENT;
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      !isMouseDownRef.current // Ngăn chặn sự lag/giật cục (rubberbanding) khi người dùng đang giữ chuột kéo (Pan)
    ) {
      if (!isMounting) {
  ...
  React.useEffect(() => {
    if (tlEditorRef.current) {
      ...
      const updatedPresences = otherCursors
        .map(({ userId, xPercent, yPercent, presenter, name, isModerator }) => {
          ...
          if (
            !active ||
            (hideViewersCursor && !isModerator && !currentUser?.presenter) ||
          ...
          tlEditorRef.current.batch(() => {
            ...
            const cameraRecords = allRecords.filter(
              (record) =>
                ...
                record.id?.split(":").pop() === formattedPageId,
            );
