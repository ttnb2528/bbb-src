...
const normalizeGetDisplayMediaError = (error) => {
  return (
    SCREENSHARING_ERRORS[error.name] ||
    SCREENSHARING_ERRORS.GetDisplayMediaGenericError
  );
};
...
const getScreenStream = async () => {
  const { constraints: GDM_CONSTRAINTS } =
...
const getNextReconnectionInterval = (oldInterval) => {
  const { mediaTimeouts: MEDIA_TIMEOUTS } =
    window.meetingClientSettings.public.kurento.screenshare;
  const {
  ...
  return loadAndPlayMediaStream(stream, mediaElement, muted).catch((error) => {
    ...
    if (error.name === "NotAllowedError") {
      ...
      throw {
        ...
        errorMessage:
          error.message ||
          SCREENSHARING_ERRORS.SCREENSHARE_PLAY_FAILED.errorMessage,
      };
    }
  });
};
...
class EXPORTED_CONFIGS {
  static BASE_MEDIA_TIMEOUT() {
    const { mediaTimeouts: MEDIA_TIMEOUTS } =
      window.meetingClientSettings.public.kurento.screenshare;
    const { baseTimeout: BASE_MEDIA_TIMEOUT } = MEDIA_TIMEOUTS;
  ...
  static BASE_RECONNECTION_TIMEOUT() {
    const { mediaTimeouts: MEDIA_TIMEOUTS } =
      window.meetingClientSettings.public.kurento.screenshare;
    const { baseReconnectionTimeout: BASE_RECONNECTION_TIMEOUT } =
      MEDIA_TIMEOUTS;
  ...
  static MAX_CONN_ATTEMPTS() {
    const { mediaTimeouts: MEDIA_TIMEOUTS } =
      window.meetingClientSettings.public.kurento.screenshare;
    const { maxConnectionAttempts: MAX_CONN_ATTEMPTS } = MEDIA_TIMEOUTS;
